...
def get_config():
  ...
  train.rng_type = 'nearby' # valid values: 'nearby', 'aoc_train'
...
class Experiment(experiment.AbstractExperiment):
  ...
  def _build_train_data(self):
    ...
    if ds_config.rng_type == 'nearby':
      generator = dataset.NearbySyntheticTripletPairsGenerator(
          min_value=ds_config.min, max_value=ds_config.max, rng_seed=self._train_seed)
    elif ds_config.rng_type == 'aoc_train':
      generator = dataset.TrainingAOCInputFileTripletPairsGenerator(
  ...
  def _build_eval_datasets(self):
    ...
    for name, min_val, max_val in zip(
      ...
      synthetic_generator = dataset.NearbySyntheticTripletPairsGenerator(
  ...
  def _build_final_eval_data(self):
    """Initializes training data."""
    aoc_input_generator = dataset.AOCInputFileTripletPairsGenerator(rng_seed=self._train_seed)
  ...
  def _initialize_model(self):
    ...
    def forward(inputs):
      inputs = hk.LayerNorm(axis=-1, create_scale=True,
  ...
  def step(self, *, global_step: jnp.ndarray, rng: jnp.ndarray, writer:
    ...
    is_logging_step = global_step % 50 == 0
    ...
    if is_logging_step:
      print(f'Loss: {loss:.6f}, Learning Rate: {learning_rate:.8f}, Accuracy: {accuracy:.2f}')
      scalars['train_accuracy'] = accuracy
    else:
